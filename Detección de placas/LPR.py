...
class LPR:
    def __init__(self, min_w=80, max_w=110, min_h=25, max_h=52, ratio=3.0769230769):
        self.min_w = min_w
        self.max_w = max_w
        self.min_h = min_h
        self.max_h = max_h
    ...
    def apply_adaptive_threshold(self, img):
        return cv2.adaptiveThreshold(img, 150, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 7, 13)
    ...
    def filter_candidates(self, contours):
        ...
        for cnt in contours:
            ...
            print(aspect_ratio)
            print(self.ratio)
            #print(np.isclose(aspect_ratio, self.ratio, atol=1))
            if (aspect_ratio>3 and w>80 and w<480 and h>50 and h<500):
            #if (np.isclose(aspect_ratio, self.ratio, atol=1) and
